function [A,E,U] = spAffinities_m( S, E, segs, nThreads,M1,alpha)
...
% indp1=find(M1>0);
% indp2=find(M2>0);
% indp=unique([indp1;indp2]);
% alpha= mean(abs(M2(indp2)-M1(indp2)));
% alpha
% alpha=0.5;
B=alpha*M1+(1-alpha)*A;
% B=B*1.5;

%% add
% B=M1.*A;

if(nargout>1), E = spDetectMex('edges',S,B); end
% A = spDetectMex('affinities',S,E,segs,nThreads);
% E = spDetectMex('edges',S,A);
